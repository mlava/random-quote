...
export default {
    ...
    */
    ...
    extensionAPI.ui.commandPalette.addCommand({
      ...
      callback: () => {
        ...
        fetchMovieQuote(uid).then(string =>
          window.roamAlphaAPI.updateBlock({
            block: {
              uid: uid,
              string: string,
  ...
  onunload: () => {
    if (window.roamjs?.extension?.smartblocks) {
      ...
      window.roamjs.extension.smartblocks.unregisterCommand("FUTURAMAQUOTE");
...
async function fetchMovieQuote() {
  let data = movieQuote.getSomeRandom(1);
